# PRISMA — Anleitung: Export Station 📦

**Dein Branch:** `feature/export-system`  
**Deine Dateien:** `src/js/modules/export-system.js`, `src/css/05-components/export-panel.css`

---

## 1. Die 4 Grundregeln (nur verstehen, nicht auswendig lernen)

1. **Jede CSS-Klasse beginnt mit `challenge-`** (z. B. `.challenge-export-panel__download-btn`) — nie `.card`, `.btn`. Ein zweites Team baut parallel, sonst ueberschreiben sich die Styles.
...
Gestern wurde alles integriert. **Dein Modul ist bereits eingebaut und laeuft.** Deine Funktionen sind in `main.js` schon eingehaengt (Copy/Export). Es gibt (noch) keine sichtbare Export-Karte.
...
```
Sagt Git „already exists"? Dann nur: `git checkout feature/export-system`

### 3.3 Arbeiten → speichern → hochladen (alle 20–30 Min wiederholen!)
```powershell
git add .
git commit -m "kurz was du gemacht hast"
git push -u origin feature/export-system
```
Das `-u origin feature/export-system` brauchst du **nur beim allerersten Mal**. Danach reicht `git push`.

### 3.4 Am Ende: Pull Request stellen
Nach `git push` zeigt GitHub oben gelb **„Compare & pull request"** → anklicken → pruefen `base: main` ← `compare: feature/export-system` → **„Create pull request"**. **Fertig fuer dich.** Du mergst NICHT selbst — das macht der Gruppenleiter.
...
- `src/js/modules/export-system.js`
- `src/css/05-components/export-panel.css`
- **Kein eigener HTML-Block:** Anders als die anderen hast du keine Karte in `index.html`. Deine Funktionen werden ueber `main.js` in die anderen Tools eingehaengt. Zum Testen: temporaerer Button oder F12-Konsole.

**NIEMALS anfassen** (gehoeren dem Gruppenleiter): `main.js`, `tokens.css`, `de.js`, `en.js`, und fremde `<article>`-Bloecke.

**Goldene Regel:** Fass `index.html` gar nicht an. Wenn du eine sichtbare Export-Karte willst, sag dem Gruppenleiter Bescheid — er baut sie ein.
...
> `copyToClipboard`, `combineCss`, `showFeedback`, `registerCssSource`, `initExportButtons`

Deine JS-Hooks (existieren schon im HTML — die KI muss GENAU diese Namen ansteuern):
  - In `main.js` sind schon registriert: `registerCssSource('colors'|'typography'|'layout', …)` und `initExportButtons()`.
  - Die anderen Module liefern ihr CSS ueber `getCurrentCss()` — du bekommst es ueber `getCssFor('colors')` etc. bereits.
...
2. **Aktuelle Datei mitgeben.** Oeffne deine `export-system.js` in VS Code, markiere alles (`Strg+A`), kopiere es, und schreib der KI: *„Hier ist die AKTUELLE Datei. Verbessere sie, erfinde nichts Neues, behalte alle Funktionsnamen:"* + einfuegen. So baut die KI auf dem Echten auf statt sich was auszudenken.
...
4. **Funktionsnamen schuetzen.** Schreib dazu: *„Diese Namen duerfen NICHT umbenannt werden: `copyToClipboard`, `combineCss`, `showFeedback`, `registerCssSource`, `initExportButtons`."*
...
9. **Nichts anderes anfassen.** *„Aendere NUR export-system.js und export-panel.css. Fass keine anderen Dateien an."*
...
```markdown
Meine Export Station (export-system.js) LAEUFT in der Basis (copyToClipboard mit
Fallback, combineCss, showFeedback, registerCssSource, initExportButtons). Ich will die
BONUS-Teile ergaenzen. (Aktuelle Datei kommt gleich.)

BONUS 1 — Datei-Download:
- downloadFile(content, filename, mimeType): erstellt einen Blob, haengt kurz ein <a>-Element
  an, klickt es, entfernt es wieder, gibt die Object-URL frei. Funktioniert fuer .css und .json.
- initDownloadButtons(): haengt an .challenge-export-panel__download-btn[data-export-format]
  (css|json). css = combineCss(...); json = Design-Tokens-Objekt
  {colors:{...}, typography:{...}, layout:{...}} aus den CSS-Variablen der Engines.

BONUS 2 — Teilbarer Link:
- buildShareableLink(state): kodiert den Zustand kompakt als Base64 im URL-Hash (#design=...),
  gibt die volle URL zurueck. parseShareableLink(): liest beim Laden #design= und gibt das
  Objekt zurueck (Anwenden uebernimmt spaeter der Gruppenleiter).
- initShareButton(): .challenge-export-panel__share-btn → Link erzeugen, in
  .challenge-export-panel__share-link anzeigen (selbst kopierbar) + in Zwischenablage.

REGELN:
- Basis-Funktionsnamen unveraendert: copyToClipboard, combineCss, showFeedback,
  registerCssSource, initExportButtons.
- Buttons brauchen aria-label; Toast-Texte ueber data-i18n
  (export.feedback.success / .error / .linkCopied / .downloaded).
- Kein Inline-Script; <a> nur programmatisch erstellen & entfernen.
- Gib mir die KOMPLETTE export-system.js zurueck, danach die komplette export-panel.css.
- Aendere NUR diese Dateien. (Wenn eine sichtbare Export-Karte noetig ist, sag ich dem
  Gruppenleiter Bescheid — du fasst index.html NICHT an.)
```

## 7. Fertig? Schnell-Check vor dem Push
- [ ] combineCss liefert einen sauberen, kommentierten CSS-Block
- [ ] showFeedback zeigt einen Toast (3s, role=status), Erfolg/Fehler farblich getrennt
- [ ] downloadFile loest einen echten Datei-Download aus (.css und .json)
